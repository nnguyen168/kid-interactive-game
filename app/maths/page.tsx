...
import MathsQuestionCard from "@/components/quiz/MathsQuestionCard";
...
export default function MathsPage() {
  ...
  const round = useQuizRound({
    total: ROUND,
    ready,
    generate: () => generateMathsQuestion(theme, progress.level.maths),
  });

  function onSolved(firstTry: boolean) {
    if (firstTry) {
      const { leveledUp, level } = addStars("maths", 1);
  ...
  return (
    ...
    >
      ...
      {round.done ? (
        ...
        round.question && (
          <MathsQuestionCard key={round.question.id} question={round.question} themeId={themeId} onSolved={onSolved} />
        )
      )}
    </GameScreen>
  );
